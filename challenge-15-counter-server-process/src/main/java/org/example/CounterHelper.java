...
public class CounterHelper {
    ...
    public CounterHelper(ShardRouter router, RedisCache cache, TaskQueue queue, EventPublisher publisher) {
        this.router = router;
        this.cache = cache;
        this.queue = queue;
    ...
    public Optional<Counter> vote(String userId, String counterId, UserVote.Vote v) {
        ...
        if (result.isPresent()) {
            ...
            // TWO downstream paths after a successful vote:
            //
            // 1) Enqueue a notification TASK on the Redis queue (challenge 14
            //    behavior — unchanged). The notification worker picks this up,
            //    handles per-task retries / dead-letter / scheduled execution.
            //    Tasks are the right shape because notifications are
            //    individual high-stakes work items that need ack semantics.
            queue.enqueue("vote-notification", new TaskMessage.VoteNotificationPayload(
                    counterId, userId, v.name(), c.getLikes(), c.getDislikes()));

            // 2) Publish a vote-cast EVENT to Kafka. Multiple consumer groups
            //    (analytics, audit, trending) each see the event and do their
            //    own work — fan-out the producer doesn't need to know about.
            //    Events are the right shape because the same fact drives
