...
public class CounterHelper {
    ...
    public Optional<Counter> vote(String userId, String counterId, UserVote.Vote v) {
        ...
        if (result.isPresent()) {
            ...
            // Enqueue a notification task. The user's vote response is sent
            // BEFORE the worker picks this up — fast response, side effect
            // happens asynchronously. If queue.enqueue() fails internally, we
            // log and continue: the vote already succeeded.
