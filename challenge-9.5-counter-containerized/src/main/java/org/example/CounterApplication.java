...
public class CounterApplication extends Application<CounterConfiguration> {
    ...
    public void initialize(Bootstrap<CounterConfiguration> bootstrap) {
        ...
        // Enable ${ENV_VAR:-default} substitution inside config.yml so the same
        // config file can be used across all containers with per-container env.
    ...
    public void run(CounterConfiguration configuration, Environment environment) {
        ...
        // Tag every request with a UUID in MDC so log lines during the request carry it.
    ...
    private void initializeDatabase(Jdbi jdbi) {
        jdbi.useHandle(h -> {
            ...
            // counters table now has a created_at column for pagination ordering.
            h.execute("""
            ...
            // Composite index supporting (created_at DESC, counter_id DESC) — the pagination ordering.
            h.execute("CREATE INDEX IF NOT EXISTS idx_counters_created_at_counter_id ON counters(created_at DESC, counter_id DESC)");
        });
        ...
        jdbi.useTransaction(h -> {
            ...
            // Seed timestamps are spaced 1 second apart so they have a deterministic order.
            // video-funny-cats is the oldest; video-music-mix is the newest.
            long base = Instant.now().getEpochSecond();
            cDao.insert("video-funny-cats",   base);
            cDao.insert("video-dev-tutorial", base + 1);
            cDao.insert("video-music-mix",    base + 2);
