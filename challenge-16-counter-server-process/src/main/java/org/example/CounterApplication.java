...
import org.example.web.BreakerResource;
import org.example.web.CircuitBreakerExceptionMapper;
...
public class CounterApplication extends Application<CounterConfiguration> {
    ...
    public void run(CounterConfiguration configuration, Environment environment) {
        ...
        // Per-shard, cache, and kafka-publish breakers. One registry per
        // counter container — state is local, no coordination across counters.
        ...
        // Breaker state goes on the ADMIN connector (port 8081), not the
        // application connector. Same reasoning as /healthcheck and /metrics:
        // operational data, internal audience. Reachable per-counter at
        // host port 18081 / 28081 / 38081 → /breakers.
        environment.admin().addServlet("breakers", new BreakerResource(breakers, mapper))
                .addMapping("/breakers");
